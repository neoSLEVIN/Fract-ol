...
/*   Updated: 2019/09/22 03:00:48 by cschoen          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	mlx_string_put(ftl->mlx_ptr, ftl->win_ptr, 5, 40 + LM, 0x00cccccc,
			"KEYBOARD:");
	mlx_string_put(ftl->mlx_ptr, ftl->win_ptr, 205, 40 + LM, 0x00cccccc,
			"NUMPAD:");
	mlx_string_put(ftl->mlx_ptr, ftl->win_ptr, 10, 60 + LM, 0x00cccccc,
			"Move: Arrows;");
	mlx_string_put(ftl->mlx_ptr, ftl->win_ptr, 210, 60 + LM, 0x00cccccc,
			"Fast move: {2, 4, 6, 8};");
...
void		draw_ui(t_frac *ftl)
{
	...
	mlx_string_put(ftl->mlx_ptr, ftl->win_ptr, 5, 2 + LM, 0x00cccccc,
			ftl->mem.ui ? "TAB - switch to keys" : "TAB - switch to info");
	if (ftl->type == MANDELBROT)
		mlx_string_put(ftl->mlx_ptr, ftl->win_ptr, 5, 20 + LM, 0x00cccccc,
			"Mandelbrot");
